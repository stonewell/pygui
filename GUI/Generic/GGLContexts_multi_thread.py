...
_current_share_group = threading.local()
_current_share_group.group = None
...
def current_share_group():
    group = _current_share_group.group
    if not group:
        raise ValueError("No current PyGUI OpenGL context:{},{}".format(threading.current_thread(), _current_share_group.group))
    return group
